...
new class extends Component {
    ...
    public $type = 'budget'; // Default to budget for this module
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between w-full">
            @foreach([1 => 'Carga', 2 => 'Resolución', 3 => 'Confirmación', 4 => 'Resultado'] as $s => $label)
                <div class="flex flex-col items-center {{ $step >= $s ? 'text-green-600' : 'text-gray-400' }}">
                    <div class="flex items-center justify-center w-10 h-10 rounded-full border-2 
                        {{ $step >= $s ? 'border-green-600 bg-green-50' : 'border-gray-300' }} font-bold">
                        {{ $s }}
                    </div>
                    <span class="text-sm mt-1">{{ $label }}</span>
                </div>
                @if($s < 4) <div class="flex-1 h-1 bg-gray-200 mx-4 {{ $step > $s ? 'bg-green-600' : '' }}"></div> @endif
            @endforeach
        </div>
    </div>
    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
        ...
        @if($step === 1)
            <div class="space-y-6">
                <div>
                    ...
                    <input type="hidden" wire:model="type" value="budget">
                </div>
        ...
        @if($step === 2)
            <div class="space-y-6">
                                            ...
                                            <tr wire:key="unknown-{{ $index }}">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    ...
                                                    >
                                                        ...
                                                        <option value="new" class="font-bold text-green-600">+ Crear como Nuevo Cliente</option>
                ...
                <div class="flex justify-end">
                    <button wire:click="nextStep" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none disabled:opacity-50">
                        Continuar
                    </button>
                </div>
        ...
        @if($step === 4)
            <div class="text-center space-y-6">
                ...
                <button wire:click="resetWizard" class="mt-6 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-green-700 bg-green-100 hover:bg-green-200 focus:outline-none">
