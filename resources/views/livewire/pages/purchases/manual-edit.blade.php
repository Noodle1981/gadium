...
new #[Layout('layouts.app')] class extends Component {
    ...
    public float $materiales_presupuestados;
    public float $materiales_comprados;
    public float $porcentaje_facturacion;

    public function mount(PurchaseDetail $purchaseDetail)
    {
        $this->purchaseDetail = $purchaseDetail;
        
        $this->moneda = $purchaseDetail->moneda;
        $this->cc = $purchaseDetail->cc;
        $this->ano = $purchaseDetail->ano;
        $this->empresa = $purchaseDetail->empresa;
        $this->descripcion = $purchaseDetail->descripcion;
        $this->materiales_presupuestados = $purchaseDetail->materiales_presupuestados;
        $this->materiales_comprados = $purchaseDetail->materiales_comprados;
        $this->porcentaje_facturacion = $purchaseDetail->porcentaje_facturacion;
    }
    ...
    public function save()
    {
        ...
        $newHash = PurchaseDetail::generateHash($this->cc, (string)$this->ano, $this->empresa, $this->descripcion);

        if ($newHash !== $this->purchaseDetail->hash && PurchaseDetail::existsByHash($newHash)) {
            $this->addError('duplicate', 'Ya existe otro registro con estos datos (CC, Año, Empresa, Descripción).');
            return;
        }
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="mb-6 flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800">Editar Compra</h2>
                        <p class="text-gray-600">Modifique los detalles del registro.</p>
                    </div>
                    <button wire:click="delete" wire:confirm="¿Está seguro de eliminar este registro?" class="text-red-600 hover:text-red-800 font-medium">
                        Eliminar Registro
                    </button>
                </div>
