...
@if(!$isManager && (auth()->user()->can('view_users') || auth()->user()->can('view_roles') || auth()->user()->can('view_hr')))
    ...
    @php
        $auditRoute = $isManager ? 'manager.audit.log' : 'admin.audit.log';
    @endphp
    <x-sidebar-link :href="route($auditRoute)" :active="request()->routeIs('*.audit.log')" icon='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>'>
        Bitácora
    </x-sidebar-link>
...
@if(auth()->user()->hasRole('Gestor de Satisfacción Personal'))
    ...
    <x-sidebar-link :href="route('staff-satisfaction.encuesta')" :active="request()->routeIs('staff-satisfaction.encuesta')" icon='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>'>
        Encuestas
    </x-sidebar-link>
@endif
